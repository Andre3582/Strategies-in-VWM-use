...
var jsPsychCopyingTask = (function (jspsych) {
  ...
  const info = {
    ...
    parameters: {
      ...
      grid_box_edge_width: {
        type: jspsych.ParameterType.INT,
        pretty_name: "Grid edge width",
        default: 4,
        description: "The edgewidth of the gridboxes.",
      },
      grid_colors: {
        type: jspsych.ParameterType.HTML_STRING,
        pretty_name: "Grid Colors",
        default: ["black", "black", "black"],
        description:
      ...
      canvas_color: {
        type: jspsych.ParameterType.HTML_STRING,
        pretty_name: "Canvas color",
        default: 'white',
        description: 'The the background color of the canvas.',
      },
      ...
      enable_covers: {
        type: jspsych.ParameterType.BOOL,
        pretty_name: "Enable covers",
        default: true,
        description: "If true, the grids are covered.",
      },      
  ...
  class CopyingTaskPlugin {
    ...
    trial(display_element, trial) {
      ...
      let canvas_height = canvas_width / trial.canvas_aspect_ratio;

      // set the grid box size as percentage of the canvas
      let grid_box_size = canvas_width * trial.grid_box_scale;
      ...
      let canvas = new fabric.Canvas("jspsych-canvas-stimulus", {
        selection: false, // Turn off the blue selection rectangle feature
        hoverCursor: "pointer",
        moveCursor: "pointer",
        backgroundColor: trial.canvas_color,
      });
      ...
      let mid_border_left = canvas_width * trial.mid_border_specs["left"];
      let mid_border_width = canvas_width * trial.mid_border_specs["width"];
      let mid_border_right = mid_border_left + mid_border_width;

      // get the number of rows and cols in the model and resource grid
      let n_model_rows = trial.model_grid_contents.length; // rows
      let n_model_cols = trial.model_grid_contents[0].length; // cols

      let n_resource_rows = trial.resource_grid_contents.length; // rows
      let n_resource_cols = trial.resource_grid_contents[0].length; // cols
      ...
      let rsc_grid_left = canvas_width * trial.grid_offset["resource_x"]; // left edge of grid at at 79% canvas width

      // workspace grid
      let wsp_grid_top = canvas_height * 0.5 - n_model_rows * grid_box_size * 0.5; // vertically centered
      let wsp_grid_left =
        mid_border_right +
      ...
      // create 2 d arrays
      let model_grid = Array.from(Array(n_model_rows), () => new Array(n_model_cols));
      let workspace_grid = Array.from(Array(n_model_rows), () => new Array(n_model_cols));
      let resource_grid = Array.from(Array(n_resource_rows), () => new Array(n_resource_cols));

      // fill the MODEL & WORKSPACE grid (arrays) with box objects and values
      for (let row = 0; row < n_model_rows; row++) {
        for (let col = 0; col < n_model_cols; col++) {
          model_grid[row][col] = {
            ...
            strokeWidth: trial.grid_box_edge_width,
          };
      ...
      for (let row = 0; row < n_resource_rows; row++) {
        for (let col = 0; col < n_resource_cols; col++) {
      ...
      let held_item_id = null;

      /////////////////////////////////////////////////
      let canvas_el = document.getElementById("jspsych-canvas-stimulus");
      let canvas_rect = canvas_el.getBoundingClientRect();
      ...
      let mid_border_rect = new fabric.Rect(mid_border_rect_params);
      ...
      for (let row = 0; row < n_resource_rows; row++) {
        for (let col = 0; col < n_resource_cols; col++) {
          if (resource_grid[row][col].content) {
            for (let rep = 0; rep < n_resource_rows * n_resource_cols + 1; rep++) {
      ...
      let placeholder_object = new fabric.Rect({
        top: canvas_height * 2,
        left: canvas_width * 2,
        width: 0,
        height: 0,
      ...
      if(trial.enable_covers){
        ...
        let mdl_cover_text = new fabric.Text('Move your mouse here\nto reveal the example.', { 
          left: mdl_grid_left + mdl_cover_board.width/2,
          originX: 'center',
          originY: 'center',
        ...
        let mdl_cover = new fabric.Group([mdl_cover_board, mdl_cover_text])
        mdl_cover.set({
          hasControls: false,
          selectable: false,
        })

        const init_mdl_cover_left = mdl_cover.left
        mdl_cover.isLocked = true; // track whether model cover is locked or open
        ...
        let rsc_cover_board = new fabric.Rect({
          ...
          height: n_resource_rows * grid_box_size, 
        })
        let wsp_cover_text = new fabric.Text('Move your mouse here\nto copy the example.', { 
          left: wsp_grid_left + wsp_cover_board.width/2,
          originX: 'center',
          originY: 'center',
        ...
        canvas.add(wsp_cover);


        // add cover functionality
        onmousemove = function(e){

          let mousePos = [e.clientX, e.clientY]

          // check if model needs to be opened
          let mouse_over_model = isOnModelSide(mousePos)
          
          if (mouse_over_model && mdl_cover.isLocked){
            //open model cover
            mdl_cover.isLocked = false;
            mdl_cover.set({left:canvas_width})
            mdl_cover.setCoords()

            // close the workspace
            wsp_cover.set({left:init_wsp_cover_left});
            wsp_cover.setCoords()

            canvas.renderAll();

            let event = {
              event: "model was opened",
          ...
          if ((! mouse_over_model)  && (! mdl_cover.isLocked) ) {
            // lock the model cover
            mdl_cover.isLocked = true;
            mdl_cover.set({left:init_mdl_cover_left})
            mdl_cover.setCoords()

            // open the workspace
            wsp_cover.set({left:canvas_width});
            wsp_cover.setCoords()

            canvas.renderAll();

            let event = {
              event: "model was closed",
              timestamp: Math.round(performance.now()),
            };
            trial_events.push(event);
          }
      ...
      function createItemFromURL(box, options_object) {
        ...
        fabric.Image.fromURL(
          ...
          function (item) {
            // Scale the item
            let true_width = item.get("width");
            let true_height = item.get("height");
            let max = Math.max(true_width, true_height);
            let scaling = (box.size / max) * trial.item_scale;
            ...
            let w = item.getScaledWidth();
            let h = item.getScaledHeight();

            // place the items on their initial position on the resource grid
            let init_pos_left = box.left + (box.size - w) * 0.5;
            let init_pos_top = box.top + (box.size - h) * 0.5;
      ...
      function CreateItemFromSVG(box, options_object) {
        let [item_shape_num, item_color_tag] = getSVGPathandColorTag(box.content);
        let path_str = trial.twenty_svg_paths[item_shape_num];
        let icon = new fabric.Path(path_str);
        ...
        let item = new fabric.Group([bg, icon]);
        // }
        ...
        //scale the object
        let w = item.get("width");
        let h = item.get("height");

        let max = Math.max(w, h);
        let scaling = (grid_box_size / max) * trial.item_scale;

        item.scale(scaling); // NOTE: objects' width and height do not change after scaling!!!!

        // set the object to the center of the grid_boxes
        let scaled_w = item.getScaledWidth();
        let scaled_h = item.getScaledHeight();

        // place the items in the correct location
        let init_pos_left = box.left + (grid_box_size - scaled_w) * 0.5;
        let init_pos_top = box.top + (grid_box_size - scaled_h) * 0.5;
      ...
      function drawGridArray(grid) {
        for (let row = 0; row < grid.length; row++) {
          for (let col = 0; col < grid[row].length; col++) {
            ...
            let rect = new fabric.Rect(rect_parameters);
            canvas.add(rect);
            box["fabric_obj"] = rect;
          }
        }
      ...
      function itemMove(e) {
        if (held_item_id === e.target.id) {
          ...
          let item_center_xy = getObjCenterXY(item);
                ...
                if (isInBox(item_center_xy, wsp_box)) {
                  ...
                  if (wsp_box.content == null) {
                    ...
                    if (item.itemType == wsp_box.correct_content || trial.allow_incorr_placement) {
                      ...
                      let l = wsp_box.left + (wsp_box.size - item.w) / 2;
                      let t = wsp_box.top + (wsp_box.size - item.h) / 2;
      ...
      // canvas_rect.top canvas_rect.left
      function isInModelGrid(pos){
        return ( 
          // (pos[0] < canvas_el.getBoundingClientRect().left + mdl_grid_left + n_model_rows * grid_box_size)
          (pos[0] >= canvas_rect.left + mdl_grid_left)  
       && (pos[0] <  canvas_rect.left + mdl_grid_left + n_model_rows * grid_box_size)
       && (pos[1] >= canvas_rect.top + mdl_grid_top)
       && (pos[1] <  canvas_rect.top + mdl_grid_top + n_model_rows * grid_box_size)
           )
      }

      function isOnModelSide(pos){
        return (pos[0] < canvas_el.getBoundingClientRect().left + mdl_grid_left + n_model_rows * grid_box_size)
      }
      ...
      const end_trial = () => {
        let endTime = Math.round(performance.now());
        ...
        let trial_completed = checkIfDone();
        ...
        let trial_data = {
          ...
          canvas_offset_top_left: [canvas_el.getBoundingClientRect().top, canvas_el.getBoundingClientRect().left],
        };
